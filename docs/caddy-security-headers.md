...
## Google Fonts is NOT allowed here either (corrected 2026-09-21)

An earlier version of this block named `fonts.googleapis.com` and
`fonts.gstatic.com` because the marketing pages once loaded their
typefaces from there. They have been self-hosted through `next/font/local`
since 2026-08-13 (CLAUDE.md, "Fonts are COMMITTED"), so no page requests
either origin and the allowance was permission nobody used. It is removed
above; all four frontends now carry the same `font-src 'self' data:`.
