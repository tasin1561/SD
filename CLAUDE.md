...
| 11 | Notifications (templates, dispatcher, throttle, BullMQ workers) | NEXT |
...
- `@skydrop/db` package: Prisma models across all 9 layers, migrations applied through M10 (M10 added `tracking_events.event_at` + the `(shipment_id, event_at DESC)` index — TRK-3), idempotent seed (system settings, couriers, FX, warehouse, rate card, 25+ notification templates, M8 warehouse-ops keys, M10 tracking keys — `tracking.webhook_secret_ref` / `tracking.public_lookup_rate_limit_per_min` / `tracking.webhook_processing_retry_max`)
...
- Test totals: **905 unit** + **81 e2e** (10 new in `tracking-flow.e2e-spec.ts`), all green; fresh-clone simulation verified.
...
**Next:** Module 11 — Notifications (templates, dispatcher, throttle, BullMQ workers). M10 left a small set of follow-ups in phase-1a-debt: (1) **NEW** `normalizeScan` raw-code table + the webhook HMAC scheme / header name — TWO `TODO(delhivery-api)` seams joining M9's existing 6 (8 total); validate against Delhivery's sandbox alongside the M9 wire seams. (2) **Deferred customer-facing tracking frontend** (`apps/track` — EN + HI) — the API is shipped; the SSR page that reads the public lookup endpoint is a frontend-cycle deliverable. (3) **NDR → call-center re-queue loop deferred to Phase 2** — currently a DELIVERY_FAILED transition just sits in the order's lifecycle; in Phase 2 it should re-enqueue the order for a call-center confirmation attempt. (4) **Manual-courier event entry UX is API-only** — the `POST /admin/tracking/shipments/:shipmentId/manual-scan` endpoint exists; the admin UI lands with M12.
