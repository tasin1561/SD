...
2. **RBAC-1 (R0): the seller guard FAILS CLOSED on writes.** `SellerJwtGuard` resolves `@SellerRoles` from the handler, else (for non-safe methods) the class-level list, else `DEFAULT_WRITE_ROLES`. Reads stay open to every role; a rejected write audits LOW `seller.access_denied_role` and 403s `INSUFFICIENT_ROLE`. A new seller WRITE endpoint is protected by default — annotate only to widen or narrow deliberately.
...
- **UI coverage audit (2026-07-28) — every controller now has a screen.** Listed every `@Controller` prefix against every page route rather than trusting the "not yet implemented" list, which had gone stale in both directions. Fifteen gaps, two of them DEAD-END WORKFLOWS where something could be started that nobody could finish:
  ...
  - **Still without UI, deliberately** — all low-value, none blocking a workflow: category attribute definitions (`admin/categories/:id/attributes`), seller CSV column mappings, saved recipient addresses, seller-side category browse. Each is a convenience over something already reachable another way.
