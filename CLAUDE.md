...
| 13 | apps/seller — second consumer of FE foundation; CP1 foundation + CP2 Orders/Catalog pattern-setters | ⏳ CP1 DONE, CP2 NEXT |
...
- **Module 13 CP1** — apps/seller foundation (the second consumer of the M12 frontend foundation). Closed three M12 deferrals: (#2) `@skydrop/ui/components` extraction — 7 admin primitives (button, card, data-table, form, modal, page, status-badge) lifted to `packages/ui/src/components/` with a new `./components` subpath export; both apps/admin (renamed imports) and apps/seller consume from the same surface (the extraction shape is PROVEN, not just designed for two); (#4) moduleResolution + dist-vs-src discipline verified by a second Next.js consumer at the same tsconfig-paths-to-src/ pattern; (#5) Playwright harness installed at the workspace root with `admin` (port 3002) + `seller` (port 3003) projects + webServer auto-spawn — 6 specs ship (login-page-renders + unauthed-/dashboard-redirect + bad-credentials-FE-2-verbatim, three per project); `pnpm e2e:fe` is the manual smoke gate, NOT a CI gate yet. The seller-side hybrid `/auth/seller/me` cookie path + `RefreshTokenService.validateByPlaintext('seller')` were ALREADY wired in M12 (no backend changes in M13 CP1). apps/seller scaffolding mirrors apps/admin exactly — Next.js 15 + Tailwind v4 + TanStack Query 5 + Geist + vitest + the same-origin `/api/[...path]/route.ts` proxy + the (authed) SSR gate calling `resolveSellerSsrIdentity` + dark-utilitarian shell with sidebar (Dashboard / Orders / Catalog / Tracking / Profile / Settings) + topbar (companyName + emailDisplay + Sign out). FE-5 identity-parameterization PROVEN in practice: the only changes vs admin are the IdentityKind parameter and the resulting types (`SellerMe` vs `StaffMe`, `__Host-sellerRefresh` vs `__Host-staffRefresh`, `resolveSellerSsrIdentity` vs `resolveStaffSsrIdentity`); the auth gate, proxy, single-flight refresh, in-memory token store, AuthProvider, useApiClient, FE-2 server-verdict-verbatim discipline are all REUSED UNCHANGED. CP1 verification documented in `apps/seller/CP1_VERIFICATION.md` (same 7-step refresh-through-proxy round-trip shape as M12's `apps/admin/CP1_VERIFICATION.md`).
- Test totals: **959 unit (API)** + **99 e2e (API)** (87 pre-M12 + 11 hybrid /me + 1 mirror) + **26 vitest (packages: api-client 16 + auth 10)** + **7 vitest (admin FE-2 boundary)** + **6 Playwright specs** (M13 CP1.7, manual smoke — admin login 3 + seller login 3), all green where exercised. CP1 manual auth round-trip + CP2 feature smoke documented (`apps/admin/CP1_VERIFICATION.md` + `apps/admin/CP2_FEATURE_SMOKE.md` + `apps/seller/CP1_VERIFICATION.md`).

**Not yet implemented:**
- Other frontends (`apps/track`, `apps/marketing`, `apps/workers`) — placeholders.
- Modules 14-18 + Reports (the original M13 slot — pushed to a later cycle since apps/seller took the M13 fork at the design phase).
- M13 CP2 — Orders pattern-setter (read-heavy: list with filters + detail with lifecycle timeline; closes the ORDER_VIEW_INCLUDE items-only debt + M11 ndr_reason debt in the same backend commit) then Catalog pattern-setter (write-heavy: variant-grain list + edit forms + image upload drag-drop multi). Placeholder pages already render through the shell at /orders /catalog /tracking /profile /settings.

**Next:** Module 13 CP2 — start with the Orders pattern-setter. The backend commit that expands `ORDER_VIEW_INCLUDE` (filtered allowlist of lifecycle event types + latest delivery_attempts) lands FIRST so the seller order-detail timeline + the M11 customer NDR `ndr_reason` template variable both light up from one PR (per the round-3 design lean #2). M11/M10 follow-ups remain open (legacy fire-once + lifecycle composite-key idempotency regimes; in-process bus → Phase-2 Redis pub/sub; Delhivery wire seams x8; etc.). M12 fast-follows still open: warehouse-ops + queue-management admin feature areas (M12 deferral #3, no architectural blockers — feature scope only).
