...
| 12 | Admin Dashboard (seller approval, order ops, warehouse ops, queue management, RBAC enforcement) | NEXT |
...
- Test totals: **959 unit** + **87 e2e** (6 new in `notifications-flow.e2e-spec.ts`), all green; force-exit warning in e2e is pre-existing (BullMQ + ioredis internal handles; forceExit:true in jest.e2e.config.ts since M1). Three consecutive full e2e runs verified post-fix.

**Not yet implemented:**
- All other apps (frontends in `apps/marketing`, `apps/seller`, `apps/admin`, `apps/track` are placeholders — M10's customer-facing EN+HI page lives in `apps/track`, deferred to the frontend cycle)
- Modules 12-18

**Next:** Module 12 — Admin Dashboard (seller approval, order ops, warehouse ops, queue management, RBAC enforcement). M11 left a small set of follow-ups in phase-1a-debt: (1) **Two idempotency regimes** on notification_logs (legacy fire-once vs M11 lifecycle composite-key) — coexist correctly; document, don't migrate legacy callers blindly. (2) **NDR `ndr_reason` template variable is empty** — M10 doesn't surface the latest delivery_attempts.reason on the order header that the listener loads; wire when M12 admin order detail surfaces it. (3) **Listener has no self-retry** — per-target failures inside the listener loop are isolated (NOTIF-3) and BullMQ retries the downstream send; the load+enqueue window itself has no retry. (4) **In-process bus** — Phase-2 multi-instance API needs a Redis pub/sub swap-in (the seam is documented). (5) **Locale is hard-coded 'en'** — the bilingual-in-one-email shape gives customers Hindi today; stored per-recipient locale is a Phase-2 customer model enhancement. (6) **Force-exit warning is pre-existing** — BullMQ + ioredis internal handles; documented, not introduced by M11. M10's previous follow-ups remain open (Delhivery wire seams x8, customer-facing tracking frontend, NDR→call-center re-queue Phase 2, manual-tracking admin UI in M12).
