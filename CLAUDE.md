...
| R6 | Multi-warehouse RTO correctness (receive-anywhere + cross-warehouse restock guard) | ✅ DONE (R6b deferred) |
...
5. **WMS-8b (R6): RTO can be RECEIVED at any ACTIVE warehouse, but a cross-warehouse RESTOCK is REFUSED, not guessed.** `stock_batches` is warehouse-scoped, so crediting the origin bin for goods standing in another building would be silent corruption. `RTO_RESTOCK_WAREHOUSE_MISMATCH` tells ops to transfer or re-inspect as WRITE_OFF. Cross-warehouse write-off is always allowed (no movement).
