...
- Test totals (2026-07-27, post-security-pass): **1497 unit (API, 136 suites)** + **127 e2e (API, 20 suites, RUN IN CI)** + **26 vitest (packages: api-client 16 + auth 10)** + **33 vitest (admin: FE-2 boundary 7 + ops primitives 17 + status-kinds/serverVerdict 9)** + **5 vitest (seller)** + **15 Playwright specs** (admin login 3 + seller login 3 + the shared nonce-CSP trio × admin/seller/track — **now a CI job**, no longer manual-only), all green. CP1 + CP2 verification docs: `apps/admin/CP1_VERIFICATION.md` + `apps/admin/CP2_FEATURE_SMOKE.md` + `apps/seller/CP1_VERIFICATION.md` + `apps/seller/CP2_FEATURE_SMOKE.md`.
