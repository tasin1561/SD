...
| R2 | Wallet accrual tiers (T+N) + seller withdrawal requests + auto-withdraw cycle | ✅ DONE (R2c deferred) |
...
- **R2c** — INSTANT_PRIORITY paid tier + the courier-settlement float ledger that would make "we can't lose 1 rupee" verifiable rather than asserted. **Note (verified 2026-07-26): there is NO record of courier→Skydrop settlement anywhere in the schema and no COD reconciliation, so today the float is real and unmeasured** — the default accrual tier is INSTANT, i.e. sellers are credited at DELIVERED, 5–10 days before the courier pays us. The free interim mitigation is flipping `wallet.accrual_timing_tier` to `T_PLUS_N` (settings only, no code).
