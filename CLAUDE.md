...
| 13 | Reports (operational, not financial) | NEXT |
...
- Test totals: **959 unit (API)** + **99 e2e (API)** (87 pre-M12 + 11 hybrid /me + 1 mirror) + **26 vitest (packages: api-client 16 + auth 10)** + **7 vitest (admin FE-2 boundary)**, all green. CP1 manual auth round-trip + CP2 feature smoke documented (`apps/admin/CP1_VERIFICATION.md` + `apps/admin/CP2_FEATURE_SMOKE.md`).

**Not yet implemented:**
- Other frontends (`apps/seller`, `apps/track`, `apps/marketing`, `apps/workers`) — placeholders. apps/seller is next module (M13 frontend cycle direction).
- Modules 13-18

**Next:** Module 13 — Reports (operational, not financial) OR apps/seller (frontend cycle pivot, leveraging the M12 foundation). M12 left a focused set of follow-ups in phase-1a-debt: (1) **`ORDER_VIEW_INCLUDE` is items-only** — no events, no delivery_attempts on the admin order detail payload. Blocks BOTH the lifecycle-events timeline on the order detail page AND the M11 `ndr_reason` debt closure (#13 from the M12 plan). They share one root: a small admin `events` include on the order view endpoint. One follow-up unlocks both. (2) **Component extraction to `packages/ui`** is intentionally deferred — apps/seller forces the shape. Until then, every component in `apps/admin/src/components/ui` is token-driven + has zero `@/...` imports so it lifts cleanly. (3) **Warehouse-ops and queue-management feature areas** in apps/admin are M12 fast-follow modules (out of M12 scope but the foundation supports them at zero additional FE cost). (4) **moduleResolution discipline** — packages use extension-less relative imports (`from './client'` not `'./client.js'`) because Next.js webpack resolves directly from src/ via tsconfig paths. When apps/seller lands, verify this discipline holds + the dist-vs-src consumption story is consistent across the two app's bundlers. (5) **Playwright e2e** — deliberately deferred to when apps/seller doubles the surface; the M12 integration coverage + manual smoke is the chosen tradeoff (CP2_FEATURE_SMOKE.md documents the rationale). M11/M10 follow-ups remain open (legacy fire-once + lifecycle composite-key idempotency regimes; in-process bus → Phase-2 Redis pub/sub; Delhivery wire seams x8; etc.).
