...
- Test totals (2026-07-27, post-courier-ops): **1476 unit (API, 134 suites)** + **99 e2e (API, RUN IN CI)** + **26 vitest (packages: api-client 16 + auth 10)** + **33 vitest (admin: FE-2 boundary 7 + ops primitives 17 + status-kinds/serverVerdict 9)** + **5 vitest (seller)** + **6 Playwright specs** (M13 CP1.7, manual smoke — admin login 3 + seller login 3), all green where exercised. CP1 + CP2 verification docs: `apps/admin/CP1_VERIFICATION.md` + `apps/admin/CP2_FEATURE_SMOKE.md` + `apps/seller/CP1_VERIFICATION.md` + `apps/seller/CP2_FEATURE_SMOKE.md`.
