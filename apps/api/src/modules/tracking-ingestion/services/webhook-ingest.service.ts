 ...
 *      (courierCode, signature). Same signature ⇒ byte-identical body
 *      (signature is a stable function of body+secret). Match ⇒ 200
 *      no-op with the original webhookId.
 ...
 *
 * Idempotency note (TRK-2): the dedup uses (courierCode, signature)
...
export class WebhookIngestService {
  ...
  async ingest(input: IngestWebhookInput): Promise<IngestOutcome> {
    ...
    // (3) TRK-2 dedup — same (courierCode, signature) ⇒ same body bytes.
    ...
    if (existing) {
      this.logger.log(
        ...
        'Duplicate webhook (matching signature already stored) — no-op',
      );
