 ...
 * A second correction while one is pending is refused rather than
...
export class StoreAddressChangeService {
  ...
  async hold(input: {
    ...
    const open = await this.prisma.client.storeAddressChangeRequest.findFirst({
      where: { orderId: input.orderId, status: StoreAddressChangeStatus.PENDING },
      select: { id: true },
    });
    if (open !== null) {
      throw new BadRequestException({
        ...
        message:
          'A correction on this order is already waiting for the seller. They have to answer that one first.',
      });
    }

    const row = await this.prisma.client.storeAddressChangeRequest.create({
      data: {
