...
import type { OrderStatus, PaymentMode, ResellerStockMode } from '@skydrop/db';
...
/**
 * RS-5 — everything the reseller MONEY needs about one order, read off the
 * order itself and never joined back to live terms: the version it was
 * placed under, the six store shares (feed to `splitFeeLines`), both
 * credit timings, and each line's transfer price and retail. Decimals,
 * never floats.
 */
...
/** The client a read runs on — the app's, or a caller's transaction. */
type Db = Pick<Prisma.TransactionClient, 'order'>;

/**
 * The ONE read of a reseller order's snapshot, as a plain function over the
 * caller's client — so the order module's `ResellerOrderReadService` and
 * the money module read it identically without either importing the other
 * (the money module sits UNDER seller-wallet-accrual, which the order
 * module imports; the R3 rule).
 *
 * Null for a channel order, and for a reseller row the CHECK constraints
 * should have refused (reported as no snapshot rather than guessed).
 */
export async function readResellerOrderSnapshot(
  db: Db,
  orderId: string,
): Promise<ResellerOrderSnapshot | null> {
