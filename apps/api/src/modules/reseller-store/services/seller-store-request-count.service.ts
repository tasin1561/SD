 ...
 * (2026-09-16).
 *
 * THREE queues stop at the same person, and they live in different
 * modules: a delivery ask (`order_delivery_action_requests`, owned by
 * `delivery-action`), an address correction
 * (`store_address_change_requests`, owned by `reseller-order`), and — since
 * 2026-09-17 — a held cancel, call-cap answer or issue with Skydrop
 * (`store_order_requests`, owned by `store-order-request`). The nav
 ...
 *
 * Reading the tables directly is therefore the only acyclic option,
 ...
 * the number. The predicates below are the SAME ones the
...
/** Held cancels, call-cap answers and issues for this seller — the same predicate. */
export function pendingStoreOrderRequestsWhere(
  sellerId: string,
): Prisma.StoreOrderRequestWhereInput {
  return { sellerId, status: StoreOrderRequestStatus.PENDING };
}

export interface StoreRequestCount {
  /** What the badge shows. The three below, added. */
...
export class SellerStoreRequestCountService {
  ...
  async forSeller(sellerId: string): Promise<StoreRequestCount> {
    const [actions, addressChanges, orderRequests] = await Promise.all([
      ...
      this.prisma.client.storeOrderRequest.count({
        where: pendingStoreOrderRequestsWhere(sellerId),
      }),
    ]);
    return {
      total: actions + addressChanges + orderRequests,
      actions,
      addressChanges,
      orderRequests,
    };
