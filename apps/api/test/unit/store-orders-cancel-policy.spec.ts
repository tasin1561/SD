...
function make(mode: ResellerStoreActionMode, order: { id: string; sellerId: string } | null) {
  ...
  return { svc, findFirst, orderWrite, policies };
}
...
describe('a store cancelling its own order', () => {
  it('DIRECT cancels through the ordinary seller path', async () => {
    ...
    await svc.cancel(USER, 'order-1', {}, CTX);
  ...
  it('ASK_SELLER refuses too, and says who does it instead', async () => {
    // No queue can hold this one: a delivery-action row needs a shipment
    // and an unpacked order has none. Refusing by name beats a button
    // that appears to work and silently drops the request.
    const { svc, orderWrite } = make(ResellerStoreActionMode.ASK_SELLER, OWNED);
    await expect(svc.cancel(USER, 'order-1', {}, CTX)).rejects.toMatchObject({
      response: { message: expect.stringContaining('seller') },
    });
    expect(orderWrite.cancelBySeller).not.toHaveBeenCalled();
  });
