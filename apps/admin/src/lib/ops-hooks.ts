...
export function useConsignee(orderId: string): UseQueryResult<ConsigneeEditability> {
  const client = useApiClient();
  return useQuery({
    queryKey: ['admin', 'consignee', orderId],
    queryFn: () => client.request<ConsigneeEditability>(`/api/admin/orders/${orderId}/consignee`),
  });
}
...
export function useConsigneeHistory(
  ...
  const client = useApiClient();
  return useQuery({
    queryKey: ['admin', 'consignee-history', orderId],
    queryFn: () =>
      client.request<readonly ConsigneeChangeRow[]>(
        `/api/admin/orders/${orderId}/consignee/history`,
      ),
  });
}
  ...
  return useMutation({
    mutationFn: ({ orderId, ...body }) =>
      client.request<{ accepted: boolean; changeId: string; message: string | null }>(
        `/api/admin/orders/${orderId}/consignee`,
        { method: 'POST', body },
      ),
    onSuccess: () => {
      void qc.invalidateQueries({ queryKey: ['admin', 'consignee'] });
      void qc.invalidateQueries({ queryKey: ['admin', 'consignee-history'] });
    },
  });
}
