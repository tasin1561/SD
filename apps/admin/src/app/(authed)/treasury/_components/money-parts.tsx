...
/**
 * Display-only pieces shared by the Money 2 area of the staff console:
 * /treasury, /bank-accounts, /liabilities, /pnl, /expenses,
 * /courier-wallet, /margin, /pricing and /reports. Nothing here fetches,
...
export function MoSection({
  ...
  return (
    <section id={id} className="mo-section">
      <SectionHeading title={title} note={note} action={action} />
      <div className="mo-card" data-flush={flush ? '1' : undefined} data-tone={tone}>
        {children}
      </div>
    </section>
  );
...
export function MoCard({
  ...
  return (
    <div
      className={className === undefined ? 'mo-card' : `mo-card ${className}`}
...
export function MetaFact({
  ...
  return (
    <span className="mo-fact" data-tone={tone}>
      {dot && <span className="mo-fact__dot" aria-hidden />}
      {children}
    </span>
  );
}
...
export function Facts({
  ...
  return (
    <dl className="mo-facts" data-cols={columns === 2 ? '2' : undefined}>
      {items.map((it, i) => (
...
function FactPair({
  ...
  return (
    <>
      <dt className={total ? 'mo-facts__total' : undefined}>{label}</dt>
      <dd className={total ? 'mo-facts__total' : undefined}>{value}</dd>
    </>
  );
}
...
export function Notice({
  ...
  return (
    <div className="mo-notice" data-tone={tone} role={role}>
      {icon !== undefined && (
        <span className="mo-notice__icon" aria-hidden>
          {icon}
        </span>
      )}
      <div className="mo-notice__body">
        {title !== undefined && <p className="mo-notice__title">{title}</p>}
        {children}
      </div>
    </div>
  );
...
export function BackLink({
  ...
  return (
    <Link href={href} className="mo-back">
      {icon}
      {children}
    </Link>
  );
...
export function AgeChip({
  ...
  return (
    <span className="mo-age sk-figure" data-tone={tone} title={title}>
