...
import { Card, CardBody, ErrorState, LoadingState, OrderStatusBadge } from '@skydrop/ui/components';
 ...
 */
export function OrderTimelineSection({ orderId }: { readonly orderId: string }): ReactElement {
  ...
  if (!events.data || events.data.length === 0)
    return (
      <Card>
        <CardBody className="text-text-muted text-sm">No events yet.</CardBody>
      </Card>
    );
