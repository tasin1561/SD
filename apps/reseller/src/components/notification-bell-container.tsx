...
/**
 * The bell in the top bar — the SAME `NotificationBell` the seller and
 * admin apps use (FE-7: one piece of chrome, not three).
 *
 * The list is only fetched when the panel opens: the count is what every
 * page needs and it is one small query, while the items are what one
 * person looks at occasionally.
 *
 * The topic CATALOGUE gives the panel its tabs and its row chips
 * (NOTIF-17). Resolved here rather than in the component, because the
 * catalogue is data and `@skydrop/ui` fetches nothing — and read from
 * the server's declared list rather than by pattern-matching a topic
