 ...
 */
export function StoreConsigneePanel({ orderId }: { readonly orderId: string }): ReactElement {
  ...
  return (
    <Section
      ...
      subtitle="The courier already has this parcel. Only they can change where it goes, and the change is stored only if they accept it."
    >
      <Card>
        <CardBody>
          <p className="text-text-muted mb-3 text-sm">{d.reason}</p>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <FormField label="Name" htmlFor="consignee-name">
              <Input
                id="consignee-name"
                disabled={!d.editable}
            ...
            <FormField label="Phone" htmlFor="consignee-phone">
              <Input
                id="consignee-phone"
                disabled={!d.editable}
            ...
            <FormField label="Routed to" htmlFor="consignee-routing">
              <Input
          ...
          <div className="mt-3">
            <Button
              ...
              onClick={() => void ask()}
            >
              {change.isPending ? 'Asking the courier…' : 'Ask the courier'}
            </Button>
          </div>

          {history.data !== undefined && history.data.length > 0 ? (
            <ul className="text-text-muted mt-4 space-y-1 text-xs">
              {history.data.map((r) => (
                <li key={r.id}>
