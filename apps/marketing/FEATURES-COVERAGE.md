...
| Our call centre confirms every COD order by phone; you see each attempt and outcome | beat `calls` — "What we discussed with your customer": No response → Confirmed with the agent's note; checklist 3; also How it works step 2; also Credentials & trust row "Confirmed by phone first" |
...
| 3A bullet (verbatim)                                                                                                              | Shown by                                                                                                                                                                                                                                                                     |
| --------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| RTO tracked to your warehouse; inspect per unit — two of an item can go different ways (restock / keep aside damaged / write off) | beats `back` (RTO initiated → RTO in transit → Received at our warehouse) and `inspect` (2 × Kurti: one to "Put back in stock", one to "Keep aside (damaged)", tray "Write off (not sellable)"); checklist 1–2; also Credentials & trust row "Every returned unit inspected" |
...
How it works (steps 1–4), the track band's sample card, the estimator, the coverage checker, the import/export panels and the Credentials & trust rows restate bullets above in courier terms; none claims more. The import/export panels are `howItWorksParcel` steps `pickup` / `border` / `lastmile` split by direction, plus "every COD order confirmed by phone" from SELLER — Orders; Delhivery and Shiprocket are named on the export panel only, because they are Indian networks. The trust row "Tracked from the courier's own scans" rests on `capabilities[3]` (live tracking) rather than a single 3A bullet. The three badge cards are `dummy()` placeholders for the owner's licences. Phase 9 checks every heading, promise and caption against the tables here.
