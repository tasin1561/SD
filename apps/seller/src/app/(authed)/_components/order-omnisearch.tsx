 ...
 * Only then are your tickets asked as well: every other search here is
 * about a parcel.
 */
 ...
 * `/seller/orders?search=` already matches the order number, their own
 * reference, the recipient's name, their phone AND the waybill — every
 * identifier somebody would be holding when they need this. A second
 * endpoint would be a second definition of "found", and the two would
 * drift the first time either gained a field.
 *
 * Scoped by the GUARD, not by this component: the seller comes off the
 * token, so there is nothing here that could reach another company's
 * orders even if it tried.
 *
...
export function OrderOmnisearch(): ReactElement | null {
  ...
  const canSee = can(identity, 'orders.view');
  const canSeeTickets = can(identity, 'tickets.view');
  ...
  // A ticket number resolves through the same list the tickets page
  // searches — one definition of "found".
  const tq = useSellerTickets(
    { search: debounced },
    { enabled: enabled && canSeeTickets && looksLikeTicketNumber(debounced) },
  );
  const tickets = useMemo(
    () => (looksLikeTicketNumber(debounced) ? (tq.data ?? []).slice(0, 5) : []),
    [tq.data, debounced],
  );

  // Cosmetic gate (FE-2) — the server refuses regardless. A search box
  // that 403s for a role which may not read orders is worse than none.
          ...
          {q.isFetching && items.length === 0 ? (
            ...
            tickets.length > 0 ? null : (
              <p className="text-text-muted px-3 py-2 text-xs">
                Nothing matches that. It searches the order number, your own reference, the
