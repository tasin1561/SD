...
import { usePayoutSchedule, useSetPayoutSchedule } from '@/lib/api-hooks';
 ...
 * two are different in kind: they decide WHEN we raise a payout request
 ...
 * (WAL-3) — minimum balance, smallest payout, per-day and per-month caps
...
export function PayoutScheduleCard(): ReactElement | null {
  ...
  async function apply(body: { autoEnabled?: boolean; hourLocal?: number }): Promise<void> {
    try {
      ...
      toast.success(
        next.autoEnabled
          ? `Automatic payouts on, at ${String(next.hourLocal).padStart(2, '0')}:00 ${next.timezone}.`
          : 'Automatic payouts off. Payouts are yours to request.',
      );
    } catch (err) {
        ...
        <div className="space-y-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="text-text-body text-sm">Automatic payouts</div>
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="text-text-body text-sm">Automatic payout hour</div>
              <p className="text-text-faint text-xs">
                ...
                {!data.autoEnabled && ' Takes effect when automatic payouts are on.'}
              </p>
            </div>
            <div className="flex items-center gap-2">
              <select
                ...
                aria-label="Automatic payout hour"
              >
          ...
          {!mayEdit && (
            <p className="text-text-faint text-xs">
              Changing these needs the payout permission. Ask an owner or admin on your team.
            </p>
          )}
        </div>
      </CardBody>
