...
export function InventoryView(): ReactElement {
  ...
  const valueCards = useMemo(() => {
    if (!summary.data) return null;
    const s = summary.data;
    const atWarehouse = Number(s.valueAtWarehouseInr);
    const inTransit = Number(s.valueInTransitInr);
    // No stock at all, priced or not — the counts above already say so,
    // and three ₹0.00 tiles would be furniture.
    if (atWarehouse === 0 && inTransit === 0 && s.valueUnknownUnits === 0) return null;
    ...
    return (
      <div className="mb-4">
        ...
        <div className="mb-4 grid grid-cols-2 gap-2 md:grid-cols-3">
          <SummaryCard
            label="Total"
            value={<Money amount={total} currency="INR" convert={false} />}
            {...(s.valueUnknownUnits > 0
              ? {
                  // Stated, never folded in as zero. A silent omission
                  // makes the total look complete and small, and there
                  // is no way to tell cheap stock from unpriced stock.
                  hint: `${s.valueUnknownUnits.toLocaleString()} unit${
                    s.valueUnknownUnits === 1 ? '' : 's'
                  } not priced — excluded`,
                }
              : {})}
          />
          <SummaryCard
            label="In India"
            value={<Money amount={s.valueAtWarehouseInr} currency="INR" convert={false} />}
            hint="On the shelf, sellable"
          />
          <SummaryCard
            label="In transit"
            value={<Money amount={s.valueInTransitInr} currency="INR" convert={false} />}
            hint="In Dhaka or in the air"
          />
        </div>
      </div>
    );
...
function SummaryCard({
  ...
  /** A node, not a string: money renders through `Money` (UI-A). */
